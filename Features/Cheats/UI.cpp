...
//Others
#include "..\Features\postprocessing\PostProccessing.h"
...
LPDIRECT3DTEXTURE9 CMenu::steam_image(CSteamID SteamId)
{
	...
	auto res = CMenu::get().device->CreateTexture(uAvatarWidth,
		uAvatarHeight,
		1,
		D3DUSAGE_DYNAMIC,
		D3DFMT_A8R8G8B8,
...
void CMenu::Draw(bool is_open) {
	...
	ImGui::MainBegin("gayassmenu", nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		PostProcessing::performFullscreenBlur(draw, alpha/255.f);
		PostProcessing::performFullscreenBlur(draw, 1.f);
		/*top textbox (logo)*/
		{
			//Logo Background
		...
		ImGui::BeginMainChild("Main", { 690, 505 + added }, false /*ImGuiWindowFlags_NoBackground*/);
		{
			...
			case Antiaim:
			{
				...
				ImGui::BeginChild("General", { 332, 250 - 35 }, false, ImGuiWindowFlags_NoBackground);
				{
					...
					ImGui::Checkbox(crypt_str("Fake Flick"), &cfg::g_cfg.antiaim.flick);
					ImGui::SliderInt(crypt_str("Tick"), &cfg::g_cfg.antiaim.flicktick, 0, 64);
				}
				ImGui::EndMenuChild();
			...
			case World:
			{
				...
				ImGui::BeginChild("View", { 332, 250 - 35 }, false, ImGuiWindowFlags_NoBackground);
				{
					...
					ImGui::Checkbox(crypt_str("Remove Animation"), &cfg::g_cfg.misc.remove_third_person_animation);
				}
				ImGui::EndMenuChild();
				ImGui::SetCursorPos({ 5, 250 + 35 });
				...
				ImGui::BeginChild("Other", { 343, 250 - 35 + added }, false, ImGuiWindowFlags_NoBackground);
				{
					ImGui::Checkbox(crypt_str("Hit Marker World"), &cfg::g_cfg.visuals.hitmarker_world);
					ImGui::ColorEdit("##hitmarker_world_color", &cfg::g_cfg.visuals.hitmarker_world_color, true);
					ImGui::Checkbox(crypt_str("Hit Marker Screen"), &cfg::g_cfg.visuals.hitmarker_screen);
					ImGui::ColorEdit("##hitmarker_screen_color", &cfg::g_cfg.visuals.hitmarker_screen_color, true);

					draw_multicombo(crypt_str("Hit Effect"), cfg::g_cfg.visuals.hiteffect, hiteffects, ARRAYSIZE(hiteffects), preview);

					if (cfg::g_cfg.visuals.hiteffect[1])
					ImGui::SliderFloat(crypt_str("Hit Effect Duration"), &cfg::g_cfg.visuals.hit_effect_duration, 0.1f, 5.f);

					ImGui::Checkbox(crypt_str("Kill Effect"), &cfg::g_cfg.visuals.kill_effect);
					ImGui::SliderFloat(crypt_str("Kill Effect Duration"), &cfg::g_cfg.visuals.kill_effect_duration, 0.1f, 5.f);

					ImGui::Checkbox(crypt_str("Damage Indicator"), &cfg::g_cfg.visuals.damage_marker);
					ImGui::ColorEdit("##dmgcolor", &cfg::g_cfg.visuals.damage_marker_color, true);
					ImGui::SliderFloat(crypt_str("Indicator Speed"), &cfg::g_cfg.visuals.damage_marker_speed, 0.0f, 5.0f);
					
					ImGui::Checkbox(crypt_str("Force Crosshair"), &cfg::g_cfg.visuals.forcecrosshair);

					draw_multicombo(crypt_str("Penetration Crosshair"), cfg::g_cfg.visuals.penetration_crosshair, pen_crosshairs, ARRAYSIZE(pen_crosshairs), preview);

					ImGui::Checkbox(crypt_str("Spread Crosshair"), &cfg::g_cfg.visuals.spread_crosshair);
					ImGui::ColorEdit(crypt_str("##spreadcrosshair"), &cfg::g_cfg.visuals.spread_crosshair_color, true);
					ImGui::Combo(crypt_str("Spread Crosshair Type"), &cfg::g_cfg.visuals.spread_crosshair_type, spreadcrosshairs, ARRAYSIZE(spreadcrosshairs));
					...
					ImGui::Checkbox(crypt_str("Local Bullet Tracers"), &cfg::g_cfg.visuals.local_bullet_tracer);
					ImGui::ColorEdit(crypt_str("##localbulltracecolor"), &cfg::g_cfg.visuals.local_bullet_tracer_color, true);
					...
					ImGui::Checkbox(crypt_str("Team Bullet Tracers"), &cfg::g_cfg.visuals.team_bullet_tracer);
					ImGui::ColorEdit(crypt_str("##teambulltracecolor"), &cfg::g_cfg.visuals.team_bullet_tracer_color, true);

					ImGui::Combo(crypt_str("Local Tracer type"), &cfg::g_cfg.visuals.local_bullet_tracers_type, tracers_type, ARRAYSIZE(tracers_type));
					ImGui::Combo(crypt_str("Enemy Tracer type"), &cfg::g_cfg.visuals.enemy_bullet_tracers_type, tracers_type, ARRAYSIZE(tracers_type));
					ImGui::Combo(crypt_str("Team Tracer type"), &cfg::g_cfg.visuals.team_bullet_tracers_type, tracers_type, ARRAYSIZE(tracers_type));
				}
				ImGui::EndMenuChild();
