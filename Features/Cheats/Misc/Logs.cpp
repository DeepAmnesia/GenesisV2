...
bool weapon_is_aim2(const std::string& weapon)
{
...
void eventlogs::events(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), crypt_str("player_hurt"))) 
	{
		...
		if (cfg::g_cfg.visuals.hiteffect[0])
		{
			if (weapon_is_aim2(weapon))
			{
				for (int i = 0; i < 15; i++) {
		...
		{
			g_ctx.local()->m_flHealthShotBoostExpirationTime() = m_globals()->m_curtime + cfg::g_cfg.visuals.hit_effect_duration;
		}
	}
