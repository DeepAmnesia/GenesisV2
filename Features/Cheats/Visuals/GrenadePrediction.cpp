...
void GrenadePrediction::Tick(int buttons)
{
	...
	act = (in_attack && in_attack2) ? ACT_DROP : (in_attack) ? ACT_THROW : (in_attack2) ? ACT_LOB : ACT_NONE;
...
void GrenadePrediction::Paint()
{
	...
	for (int i{ 1 }; i <= m_globals()->m_maxclients; ++i) {
		...
		if (weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE) {
			...
			Vector vPelvis = player->hitbox_position(CSGOHitboxID::Pelvis);
	...
	if (target.second && weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE && (!cfg::g_cfg.visuals.on_click || g_ctx.get_command()->m_buttons & IN_ATTACK || g_ctx.get_command()->m_buttons & IN_ATTACK2)) {
		Vector screen;
		...
		if (math::world_to_screen(target.second->hitbox_position(CSGOHitboxID::Head), screen))
			renderer::get().text(renderer::get().esp, screen.x - 4, screen.y - 28, (int)target.first >= target.second->m_iHealth() ? Color::Red : Color::White, true, true, false, true, std::to_string((int)target.first).c_str());
	}
	...
	if (type && path.size() > 1)
	{
		...
		for (auto it = path.begin(); it != path.end(); ++it)
		{
			...
			{
				renderer::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, cfg::g_cfg.visuals.grenade_prediction_tracer_color, 1.f);
				Color clr2 = Color(cfg::g_cfg.visuals.grenade_prediction_tracer_color.r() / 255.f, cfg::g_cfg.visuals.grenade_prediction_tracer_color.b() / 255.f, cfg::g_cfg.visuals.grenade_prediction_tracer_color.g() / 255.f, 0.2f);
				renderer::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, clr2, 7.f);
			}
		...
		//	renderer::get().text(renderer::get().grenades, nadeEnd.x - 8, nadeEnd.y - 5, Color(cfg::g_cfg.visuals.grenade_prediction_tracer_color.r() / 255, cfg::g_cfg.visuals.grenade_prediction_tracer_color.g() / 255, cfg::g_cfg.visuals.grenade_prediction_tracer_color.b() / 255, 255 / 255), true, true, false, true, weapon->get_icon());

		Vector endpos = path[path.size() - 1];

		//circle
		if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
			renderer::get().Draw3DFilledCircle(endpos, 144, cfg::g_cfg.visuals.grenade_prediction_tracer_color);
		else if (weapon->m_iItemDefinitionIndex() == WEAPON_MOLOTOV || weapon->m_iItemDefinitionIndex() == WEAPON_INCGRENADE)
			renderer::get().Draw3DFilledCircle(endpos, 150, cfg::g_cfg.visuals.grenade_prediction_tracer_color);
		else if (weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE)
			renderer::get().Draw3DFilledCircle(endpos, 55, cfg::g_cfg.visuals.grenade_prediction_tracer_color);
	}
