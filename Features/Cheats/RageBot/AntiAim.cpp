...
{
	if (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 || g_ctx.globals.weapon->is_grenade())
		return;

	if (!cfg::g_cfg.antiaim.flick)
		return;
	...
	if (m_globals()->m_tickcount % cfg::g_cfg.antiaim.flicktick - 1 == 0)
		g_ctx.globals.tickbase_shift++;

	if (m_globals()->m_tickcount % cfg::g_cfg.antiaim.flicktick == 0) 
	{
		flicker = true;
