...
void render::rect_world(Vector center, Vector2D size, Color color, int angle) {
	Vector top_left, top_right, bot_left, bot_right;
...
//    Vector c_top_left = center + add_top_left;
	...
	if (math::world_to_screen(c_top_left, m_flTopleft) && math::world_to_screen(c_top_right, m_flTopRight) &&
		...
		line(m_flTopleft.x, m_flTopleft.y, m_flTopRight.x, m_flTopRight.y, color);
		line(m_flTopRight.x, m_flTopRight.y, m_flBotRight.x, m_flBotRight.y, color);
		line(m_flBotRight.x, m_flBotRight.y, m_flBotLeft.x, m_flBotLeft.y, color);
		line(m_flBotLeft.x, m_flBotLeft.y, m_flTopleft.x, m_flTopleft.y, color);
		//	m_surface()->DrawTexturedPolygon(4, vertices, true);
	}
}
