...
static const char* all_player_model_indexes[] =
{
...
void InitCustomModels()
{
	if (!LoadPlayerMdlOnce)
	{
		...
		{
		case 2:
			player_ml = cfg::g_cfg.player.player_model_t;
			break;
		case 3:
			player_ml = cfg::g_cfg.player.player_model_ct;
			break;
		}
		...
		// Danger Zone
		for (int i = 0; i < ARRAYSIZE(all_player_model_indexes); i++) {
			prechace_model(all_player_model_indexes[i]);
			//L::Print("Prechaced model id:" + std::to_string(i));
		}
...
std::vector <SkinChanger::PaintKit> SkinChanger::skinKits2;
std::vector <SkinChanger::PaintKit> SkinChanger::gloveKits;
std::vector <SkinChanger::PaintKit> SkinChanger::gloveKits2;
...
static attributableitem_t* make_glove(int entry, int serial) noexcept
{
	...
	static auto Fn = util::find_pattern(CLIENT_DLL, crypt_str("\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x57\x8B\xF1"), crypt_str("xxxxxxxxxxxx"));
...
void SkinChanger::run(ClientFrameStage_t stage) noexcept
{
	...
	if (true)
	{
		const char** player_model_index = all_player_model_indexes;
		auto player_model = 0;

		switch (g_ctx.local()->m_iTeamNum())
		{
		case 2:
			player_model = cfg::g_cfg.player.player_model_t;
			break;
		case 3:
			player_model = cfg::g_cfg.player.player_model_ct;
			break;
		}
