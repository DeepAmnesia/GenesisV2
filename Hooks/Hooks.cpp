...
namespace hooks
{
	...
	DWORD original_set_viewmodel_offsets;
	DWORD original_viewmodel_fov;
	DWORD original_shouldskipanimframe;
}

typedef int(__cdecl* hk_set_viewmodel_offsets_fn)(uintptr_t, uintptr_t, int, float, float, float);
int __fastcall hooks::hk_set_viewmodel_offsets(uintptr_t ecx, uintptr_t edx, int something, float x, float y, float z)
{
	if (!m_engine()->IsInGame() || !m_engine()->IsConnected())
		return (hk_set_viewmodel_offsets_fn(original_set_viewmodel_offsets)(ecx, edx, something, x, y, z));

	if (!g_ctx.local() || !g_ctx.local()->is_alive())
		return (hk_set_viewmodel_offsets_fn(original_set_viewmodel_offsets)(ecx, edx, something, x, y, z));

	if (g_ctx.globals.scoped)
		return (hk_set_viewmodel_offsets_fn(original_set_viewmodel_offsets)(ecx, edx, something, x, y, z));

	x = cfg::g_cfg.visuals.viewmodel_x;
	y = cfg::g_cfg.visuals.viewmodel_y;
	z = cfg::g_cfg.visuals.viewmodel_z;
	

	return (hk_set_viewmodel_offsets_fn(original_set_viewmodel_offsets)(ecx, edx, something, x, y, z));
}

float_t __fastcall hooks::hkGetViewmodelFOV(LPVOID pEcx, uint32_t)
{
	return cfg::g_cfg.visuals.viewmodel_fov;
}
