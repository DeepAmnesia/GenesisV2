...
void remove_smoke()
{
	if (true && cfg::g_cfg.visuals.removals[REMOVALS_SMOKE])
	{
		static auto smoke_count = *reinterpret_cast<uint32_t**>(util::FindSignature(CLIENT_DLL, crypt_str("A3 ? ? ? ? 57 8B CB")) + 0x1);
		*(int*)smoke_count = 0;
	}

	if (g_ctx.globals.should_remove_smoke == true && cfg::g_cfg.visuals.removals[REMOVALS_SMOKE])
		return;

	g_ctx.globals.should_remove_smoke = true && cfg::g_cfg.visuals.removals[REMOVALS_SMOKE];
...
void __stdcall hooks::hkFrameStageNotify(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_START)
	{
		...
		if (cfg::g_cfg.visuals.removals[REMOVALS_FLASH] && g_ctx.local()->m_flFlashDuration() && true) //-V807
			g_ctx.local()->m_flFlashDuration() = 0.0f;

		if (*(bool*)m_postprocessing() != (true && cfg::g_cfg.visuals.removals[REMOVALS_POSTPROCESSING] && (!cfg::g_cfg.visuals.world_modulation || !cfg::g_cfg.visuals.exposure)))
			*(bool*)m_postprocessing() = true && cfg::g_cfg.visuals.removals[REMOVALS_POSTPROCESSING] && (!cfg::g_cfg.visuals.world_modulation || !cfg::g_cfg.visuals.exposure);

		if (cfg::g_cfg.visuals.removals[REMOVALS_RECOIL] && true)
		{
		...
		if (g_ctx.local()->is_alive())
		{
			...
			if (true && cfg::g_cfg.visuals.removals[REMOVALS_SCOPE])
			{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		{
			if (cfg::g_cfg.visuals.nightmode && true)
				nightmode::get().apply();
			else
		...
		if (true && cfg::g_cfg.visuals.removals[REMOVALS_ZOOM])
