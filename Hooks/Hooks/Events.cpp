...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (true && cfg::g_cfg.visuals.server_bullet_impacts)
			...
			//if (true && cfg::g_cfg.visuals.local_bullet_tracer)
				//bullettracers::get().RenderBeam(false, HeadPosition, position, cfg::g_cfg.visuals.local_bullet_tracer_color, false);
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (cfg::g_cfg.visuals.kill_effect)
			{
				g_ctx.local()->m_flHealthShotBoostExpirationTime() = m_globals()->m_curtime + cfg::g_cfg.visuals.kill_effect_duration;
			}
		}
	}
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (hurt && attackerE) {
			...
			if (cfg::g_cfg.visuals.damage_marker) 
			{
				if (hurt != g_ctx.local() && attackerE == g_ctx.local()) 
				{
					...
					*/
					...
					hitmarker::get().add_hit(hitmarker_t(m_globals()->m_curtime + 2.f, user_id, event->GetInt("dmg_health"), HITGROUP_HEAD, hurt->hitbox_position_matrix(get_hitbox_by_hitgroup(event->GetInt("hitgroup")), entity->m_CachedBoneData().Base())));
				}
			}
		}
	...
	/*if ((cfg::g_cfg.visuals.bullet_tracer || cfg::g_cfg.visuals.enemy_bullet_tracer) && true)
		bullettracers::get().events(event);*/
