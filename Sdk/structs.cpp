...
Vector player_t::get_eye_position()
{
	if (!this) //-V704
		return ZERO;

	static auto sig = reinterpret_cast<float* (__thiscall*)(void*, Vector*)>(util::FindSignature("client.dll", "55 8B EC 56 8B 75 08 57 8B F9 56 8B 07 FF 90 ? ? ? ?")); // 8B 07 FF 90 ? ? ? ? 80 BF ? ? ? ? ? 74 10 
	Vector eye_pos;
	sig(this, &eye_pos);
	return eye_pos;
}
